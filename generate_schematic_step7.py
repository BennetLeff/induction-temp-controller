#!/usr/bin/env python3
"""
STEP 7: DC Power Input Section
Add USB-C connector (J1) with bulk capacitor (C1)
This is the start of the real circuit topology
"""
...
# Add connector symbol to our library
SYMBOLS = """
...
# Layout - left side DC power input
j1_x = grid(25)
j1_y = grid(30)
...
schematic = f"""(kicad_sch
  (version 20231120)
  (generator "step7-dc-input")
  (generator_version "1.0")
  (uuid "{guid()}")
  (paper "A3")
  (title_block
    (title "Induction Temp Controller - Step 7")
    (date "2025-01-01")
    (rev "1.0")
    (comment 1 "DC Power Input Section")
  )
...
print("✅ STEP 7: DC Power Input Section!")
print(f"\nCircuit:")
print(f"  USB-C J1 → C1 (100µF) → +5V rail")
print(f"           → GND")
print(f"\nSignal flow:")
print(f"  Pin 1 (5V)  → C1 pin 1 → +5V")
print(f"  Pin 2 (GND) → C1 pin 2 → GND")
print(f"\nComponents:")
print(f"  J1: USB-C connector")
print(f"  C1: 100µF bulk capacitor (filtering)")
print(f"  Power rails: +5V and GND")
